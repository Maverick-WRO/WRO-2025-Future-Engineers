...
#define IN2_PIN           10   // Motor direction pin 2 (analog 3 as digital)
...
// MPU6050 pins are fixed to A4(SDA), A5(SCL) on Arduino Uno
...
int fixing_time = 0;  // <-- Declare fixing_time globally to fix scope error
...
void setup() {
  ...
  steeringServo.write(INITIAL_ANGLE);
...
void MoveFWwithGyro_Reverse() {
  ...
  float servo_angle = constrain(INITIAL_ANGLE + output, 60, 140);
...
// Helper: general ultrasonic read for given pins
int ultrasonicDistance(int trigPin, int echoPin) {
  ...
  if (duration == 0)
    return 200; // Max distance (no echo)
